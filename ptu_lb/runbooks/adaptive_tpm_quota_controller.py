...
# N리전 확장 시에는 region_max(n) 으로 (n-1)*MIN 을 차감해 일반화한다.
MAX_CAPACITY = TOTAL_CAPACITY_UNITS - MIN_CAPACITY   # 2리전 기본값 =15
...
def region_max(n: int) -> int:
    """N리전 일반화된 리전별 최대 capacity(= 총량 − 나머지 (n-1) 리전 최소 보장).

    한 리전이 최대를 가져가도 나머지 리전이 각각 MIN_CAPACITY 를 확보할 수 있어야
    하므로 상한은 TOTAL − (n-1)*MIN 이다. n=2 이면 TOTAL − MIN 로 기존과 동일.
    """
    return max(MIN_CAPACITY, TOTAL_CAPACITY_UNITS - (max(n, 1) - 1) * MIN_CAPACITY)


def even_split_targets(regions: list[str]) -> dict[str, int]:
    """RI(Reserved) 총량을 리전 수로 균등 분배(내림 + 나머지 앞 리전부터).

    메트릭 조회 실패 등 판단 근거가 열화됐을 때, 미할당 버퍼로 PTU 를 낭비하지 않고
    RI 에 맞는 PTU 를 최대 소진하기 위한 **fallback 목표**다. 합은 항상 TOTAL 이다.
    """
    n = len(regions) or 1
    base = TOTAL_CAPACITY_UNITS // n
    rem = TOTAL_CAPACITY_UNITS - base * n
...
def need_capacity(consumed_tpm: float, cap_max: int = MAX_CAPACITY) -> int:
    """AIM_UTIL 에서 돌릴 수 있는 capacity 를 역산(리전 최소/최대로 clamp).

    cap_max 는 리전 수에 따른 상한(region_max(n))으로, 기본값은 2리전 기준이다.
    """
    if consumed_tpm <= 0:
        return MIN_CAPACITY
    raw = math.ceil(consumed_tpm / (AIM_UTIL * TPM_PER_CAPACITY_UNIT))
    return clamp(raw, MIN_CAPACITY, cap_max)
...
def compute_targets(states: list[RegionState]) -> tuple[dict[str, int], dict[str, int], bool]:
    ...
    for s in states:
        ...
        targets[s.region] = clamp(MIN_CAPACITY + extra, MIN_CAPACITY, cap_max)
    ...
    for s in sorted(states, key=lambda x: x.consumed, reverse=True):
        while leftover > 0 and targets[s.region] < cap_max:
            targets[s.region] += 1
            leftover -= 1
...
def run_cycle(dry_run: bool) -> list[RegionResult]:
    ...
    for region, cfg in REGIONS.items():
        ...
        except Exception:  # noqa: BLE001 - 메트릭 열화 시 fallback 으로 진행
            consumed = 0.0
    ...
    # 정상: 소비 비율 재분배. Fallback: 메트릭 열화 시 RI(Reserved) 총량을 균등분배해
    # 미할당 버퍼 없이 RI 에 맞는 PTU 를 최대 소진한다(감축→증설 순서는 그대로 유지).
    fallback = bool(degraded)
    if fallback:
        targets = even_split_targets([s.region for s in states])
        _needs = dict(targets)
    ...
    for s in states:
        ...
        if fallback:
            reason += " | FALLBACK RI 균등분배(메트릭 열화)"
