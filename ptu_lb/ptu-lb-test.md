...
| MAX_CAPACITY | 리전별 최대 capacity(= TOTAL − MIN_CAPACITY) | 15 |
| COOLDOWN_MINUTES | 조정 후 재조정 금지 시간(분) | 20 |

> 불변식: 항상 `cap_KR + cap_UK ≤ TOTAL_CAPACITY_UNITS`. 2리전 가정에서 리전 최대는 `TOTAL − MIN_CAPACITY`이며, 최소는 총량의 25%를 보장한다.
...
2. 기울기 30분: Korea util 85%+ / UK util 20% → UK 먼저 감축 후 Korea 증설(감축→증설)
