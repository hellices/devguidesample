...
> A10과 T4의 **택일 문제가 아니다.** 기본 구성은 A10 × N(현행 A10 × 2)이며, 본 섹션은 **A10 물량 확보가 불가한 경우 T4를 몇 대로 맞춰야 동등 처리량이 나오는가**에 대한 대체 산정 기준이다. vCPU 교차검증(GPU compute-bound 확인)이 근거 — T4 계열 최소 SKU(NC4as_T4_v3, 4 vCPU)로도 GPU 처리량은 동일하므로 **처리량/달러 최적점 = 최소 SKU × N**.
...
**정리:** 기본은 A10 × N 구성을 유지하되, **A10 물량 확보가 불가한 시점에는 T4 × 5N(500자 기준, 1,000자 위주면 6N)으로 대체해도 처리량·비용 모두 등가 수준을 확보**할 수 있다. LB 등 엔드포인트 구조는 기존과 동일하므로 backend pool 교체만으로 전환 가능. 노드 수 증가에 따른 관리 포인트 확대는 VMSS 자동화로 완화. 실환경 편차(LB 분배·warmup·eviction)를 감안해 여유분 +1대 이상 산정을 권장.
...
- 1M+ 청크 적재: A10 1대 ≈ T4 5대 — 기본은 A10, A10 수급 불가 시 T4 × 5N으로 대체 산정 → [대체 전략](#대체-전략-a10-수급-불가-시-t4-스케일아웃) 참고
