...
쿼리 경로는 latency-bound — 단건 5.59ms(T4)는 GPU 유휴 시의 하한선으로, 증설로 낮아지지 않음. 증설이 유효한 구간은 **GPU 포화 접근으로 큐잉 지연이 발생하는 시점**:

- 메커니즘: TEI dynamic batching에서 도착률이 처리 상한에 근접하면 배치 대기 시간이 비선형 증가 (큐잉 특성상 utilization ~80%부터 급증). 큐잉 지연은 노드당 도착률에 비례하므로 **노드 증설로 선형 해소 가능**
- 노드당 수용량: 500자 실측 상한 ~60 texts/s 기준 보수 산정 시 **T4 1대 ≈ 60 QPS 상한, 안전 운용 ~40 QPS/대** (utilization ~70%). 실제 쿼리는 10~100자로 짧아 O(N²) attention·토큰 수 비례상 상한이 이보다 높을 가능성 — 단, 단문 QPS는 본 측정 범위 외

증설 트리거 (권장):

| 지표 | 임계값 | 의미 |
|------|--------|------|
| GPU utilization (지속) | > 70% | 포화 접근 — 큐잉 시작 전 선제 증설 |
| 임베딩 p95 레이턴시 | 베이스라인(≈6ms)의 2~3× | 큐잉 지연 발생 중 |
| 노드당 QPS | > 40 (500자 등가 보수 기준) | 안전 마진 소진 |
| TEI 큐 메트릭 (`te_queue_size`) | 지속 증가 | 배치 대기 적체 |

- 가장 실용적 지표는 **p95 레이턴시** — 베이스라인 대비 2~3× 상승 시 큐잉으로 판단, VMSS 구성이면 GPU util 70% 기준 autoscale 룰로 자동화 가능
- **쿼리·적재 혼용 주의**: 동일 풀에서 Push 적재(대형 배치)와 쿼리를 겸용하면 적재 배치의 GPU 점유로 쿼리 레이턴시가 상승. 이 경우 증설보다 ① 적재 시간대 분리 또는 ② backend pool 분리(쿼리 전용 + 적재 전용)가 우선 — 쿼리 트래픽 단독으로 40 QPS/대를 초과하는 경우보다 적재 간섭이 원인인 경우가 흔함
