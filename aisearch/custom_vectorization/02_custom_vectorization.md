...
TEI가 sentence-transformers보다 유리한 구조적 이유(dynamic batching, Rust 네이티브 동시성, GPU 최적화)는 [Custom 임베딩 적재 가이드 — 서빙 엔진 비교](01_custom_embedding_guide.md#3-gpu-임베딩-서빙-엔진-tei-vs-vllm) 참고. 아래는 이 가이드(CPU 기본 구성) 기준의 실용적 차이:
...
- [GPU vLLM RAG 가이드](03_gpu_vllm_rag_guide.md) — T4 GPU에서 vLLM + Push API 대량 적재 + 하이브리드 검색
