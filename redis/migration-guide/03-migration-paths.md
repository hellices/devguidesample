...
왜 하필 절반인가. `SCAN`은 키스페이스를 커서로 한 번 훑는다.
...
**여기서 중요한 것**은 반복 패스가 **유실을 줄이지만 다운타임은 줄이지 못한다**는 점이다.
...
- **VNet 주입 캐시 미지원**: 같은 이유로 대상 밖이며, 검증 단계에서 경고가 아니라 오류로 걸린다.
...
2절에서 측정한 111초(3.77GB / 215만 키)는 **복사 방식의 하한**이다. 그 아래로 내려가려면 복사가 아니라
...
트래픽이 도는 대로 뜨거운 키가 자연스럽게 AMR로 넘어간다. **벌크 복사 자체가 필요 없어진다.**
...
**주의할 점은 미러 트래픽이 fire-and-forget이라는 것이다.** 응답을 기다리지 않으므로 **AMR 쓰기 실패가 아무 데도 드러나지 않는다.**
Envoy 문서도 이 필터를 "not hardened"로 표기한다. 대조 검증을 반드시 별도로 돌려야 한다.

트래픽 경로에 홉이 하나 늘어난다는 점, 그리고 프록시 자체가 새로운 단일 장애점이 된다는 점도 계산에 넣어야 한다.
...
표를 세로로 훑으면 **다운타임과 유실이 동시에 0인 줄이 없다**는 것이 보인다.
