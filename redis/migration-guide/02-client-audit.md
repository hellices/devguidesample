...
정책을 무엇으로 고르든, 명령어를 하나도 안 고치든 바뀌는 것들이 먼저 있습니다.
선택의 여지가 없는 항목이라 앞에서 털고 갑니다.
...
1절이 "어디로 붙는가"였다면 여기서는 "무엇으로 붙는가"입니다.
그런데 이 질문의 답을 정하는 것은 SDK가 아니라 **`clusteringPolicy`입니다** — 같은 SDK라도
`EnterpriseCluster`로 가느냐 `OSSCluster`로 가느냐에 따라 할 일이 전혀 달라집니다.
...
스크립트가 매기는 등급은 넷입니다. 등급이 나뉘는 기준은 심각도가 아니라 **손댈 수 있는 시점**입니다 —
TIER 1은 정책을 정하기 전에 코드가 먼저 고쳐져야 하고, TIER 2와 3은 그 정책 선택의 입력값이 되며,
TIER 4는 운영 절차 쪽에 가깝습니다.
...
여기 있는 둘은 `NoCluster`를 골라도, 해시 태그를 다 붙여도 남습니다.
정책이 정하는 것은 클러스터를 클라이언트에게 어떻게 보여 줄지인데, 이 둘은 클러스터의 문제가 아니기 때문입니다.
**코드를 고치는 것 말고는 방법이 없습니다.**

코드에서 무엇을 찾을지부터 정리하면 이렇습니다.

| 확인 대상 | 무엇을 찾나 |
|---|---|
| **다중 데이터베이스** | `SELECT n` (n≥1), `MOVE`, `SWAPDB`, 커넥션 문자열 끝의 `/1`~`/63`, Spring `spring.redis.database`, Lettuce/Jedis의 `database` 옵션, `redis://host:6380/2` |
| **키스페이스 알림** | `__keyspace@0__:`, `__keyevent@0__:expired`, `notify-keyspace-events`, `RedisIndexedSessionRepository`, `@EnableRedisIndexedHttpSession` |

**다중 DB부터 봅니다.** AMR이 주는 데이터베이스는 0번 하나뿐인데, ACR은 SKU에 따라 16~64개를 씁니다
(C0~C3·P1 16개, C4·P2 32개, C5·P3 48개, C6·P4·P5 64개). 1번 이상을 하나라도 쓰고 있으면 그대로 깨집니다.
...
**키스페이스 알림은 이 가이드에서 문서와 실측이 갈린 유일한 항목**이라 길게 적습니다.
문서가 미지원이라고 한 기능이 실제로는 동작하는데, 그렇다고 써도 된다는 뜻은 아니기 때문입니다.
...
4절이 "정책으로는 안 되는 것"이었다면 여기는 **정책으로 절반쯤 되는 것**입니다.
`EnterpriseCluster`를 고르면 다중 키 명령이 대체로 그대로 도는데, 서버가 크로스 슬롯으로 통과시키는 것은
6개(`DEL`·`MSET`·`MGET`·`EXISTS`·`UNLINK`·`TOUCH`)뿐이라 그 밖은 여전히 실패합니다.

[ACR과 AMR의 차이 2.4절](01-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)에서 목록 밖 24개를 실제로 돌려 봤고
전부 `CROSSSLOT`으로 실패했습니다. 아래 표는 그 결과를 계열별로 넓힌 것이니,
**자기 코드에 있는 명령이 여기 걸리는지만 확인하면 됩니다.**
...
앞의 두 등급이 "어떻게든 고쳐야 하는 것"이었다면 여기 둘은 조건부입니다.
TIER 3은 정책을 무엇으로 고르느냐에 따라 문제가 되기도 하고 안 되기도 하고,
TIER 4는 애플리케이션 코드보다 운영 절차 쪽에서 걸립니다.
...
3절의 스크립트를 돌려 TIER 1이 0건으로 나왔다면, 그 결과를 어디까지 믿어도 되는지가 다음 질문입니다.
답은 **"명령 이름이 소스에 글자로 남아 있는 만큼만"** 입니다. 구조적으로 놓치는 것이 셋 있습니다.
...
세 번째까지 오면 소스를 아무리 뒤져도 답이 안 나옵니다.
