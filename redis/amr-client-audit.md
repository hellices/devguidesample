...
관련 문서: [ACR과 AMR의 차이](amr-differences.md) · [이관 경로와 실측](amr-migration-paths.md)
...
> **이 랩에서 실제로 접속해 본 것은 `redis-py` 하나뿐입니다** (비클러스터·클러스터 양쪽 모두, [2.4절](amr-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)).
...
- **비클러스터 클라이언트를 그대로 두면 안 됩니다.** 연결도 되고 `SET`/`GET`도 되지만
  다중 키 명령이 커넥션 단위로 `MOVED`를 냅니다 ([2.4절](amr-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)).
...
> 마이그레이션 도구를 쓸 때도 [7절](amr-migration-paths.md#7-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다) 표에 있듯
...
아래는 전부 그 밖입니다. [2.4절](amr-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)에서 목록 밖 24개를 실제로 돌려 봤고
...
1. **해시 태그로 같은 슬롯에 모읍니다.** ([2.2절](amr-differences.md#22-샤딩과-클러스터--amr은-항상-클러스터입니다))
