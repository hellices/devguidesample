...
- 검증 방법 자체에도 함정이 셋 있습니다 — `EXISTS`로 유실 세기, `DUMP` 페이로드 바이트 비교,
  `SCAN` 앞부분에서 표본 뽑기. `verify_migration.py`는 셋 다 피해 가도록 짜여 있고,
  왜 그래야 하는지는 [이관 경로와 실측 6.2절](../migration-guide/03-migration-paths.md#62-재현하기)에 관측치와 함께 적어 두었습니다.
