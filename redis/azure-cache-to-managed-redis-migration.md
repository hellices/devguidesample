...
ACR과 AMR은 이름이 비슷하고 같은 Redis API를 말하지만 **다른 소프트웨어**입니다.
ACR(Basic/Standard/Premium)은 OSS Redis 위에, AMR은 Redis Enterprise 스택 위에 올라가 있습니다.
이 마이그레이션에서 겪는 일의 대부분이 이 한 문장에서 나옵니다.

경로를 고르는 기준은 공식 문서의 **Option 1(자체 마이그레이션)** 이고 이 문서도 그 전제로 썼습니다.
Option 2인 Azure 마이그레이션 도구는 이름에서 기대하게 되는 것과 달리 **데이터를 옮기지 않습니다** —
호스트 이름만 넘깁니다. 어떤 조건에서 쓸 만한지는 [4절](#4-경로-선택--option-1이-기본)에 있습니다.

그래서 먼저 정할 것이 셋입니다. 앞의 결정이 뒤를 제약하므로 순서도 이대로입니다.
...
숫자를 먼저 보면 이 마이그레이션의 모양이 대강 잡힙니다.
**쓰기를 멈추면 2분 안에 무손실로 끝나고, 멈추지 않으면 절반가량을 잃습니다.**
그 사이의 중간 지대가 생각보다 좁다는 것이 아래 표의 요지입니다.
...
48.47%라는 숫자보다 그 옆 칸을 더 눈여겨보세요. **키 개수 검증은 통과했습니다.**
유실이 조용히 지나간다는 것이 이 문서에서 가져갈 것이 하나뿐이라면 그 하나입니다.
측정 방법과 원본 데이터는 [이관 경로와 실측](migration-guide/03-migration-paths.md)에 있습니다.

### 반드시 알고 갈 차이

다른 소프트웨어라는 사실이 실무에서 어떤 모습으로 나타나는지를 한 화면에 모았습니다.
**첫 줄이 나머지를 거의 다 설명합니다** — AMR은 SKU와 무관하게 항상 클러스터이고,
그래서 클러스터를 켜 본 적 없는 워크로드도 크로스 슬롯 제약을 받게 됩니다.
...
마지막 줄이 계획을 가장 크게 흔듭니다. "복제를 걸어 두고 따라잡으면 무중단으로 되지 않나"는
가장 자연스러운 발상인데, 이 구간에서는 그 길이 아예 없습니다. 왜 없는지는
[이관 경로와 실측 4.1절](migration-guide/03-migration-paths.md#41-가장-먼저-떠오르는-방법-그리고-왜-막히는가)에서 양쪽을 다 확인했습니다.
...
판단에 필요한 것만 이 문서에 두고, 근거와 절차는 세 문서로 내려 두었습니다.
세 문서는 순서대로 읽어도 되지만, 대개는 지금 하고 있는 일에 맞는 하나만 펼치게 됩니다.
...
위 분기를 조건별로 편 것이 아래 표입니다. 자기 경우에 해당하는 한 줄만 보면 됩니다.
...
> 비클러스터 클라이언트로도 연결은 되고, **샤드가 하나면** `SET`/`GET`까지 동작해서 괜찮아 보입니다.
> 그런데 다중 키 명령은 그 구성에서도 **커넥션 단위로 갈려서** 풀의 일부만 계속 실패하고,
> 샤드가 둘 이상이면 단일 키 명령까지 `MOVED`로 실패합니다
...
이 둘은 거의 항상 함께 요구받습니다. 그래서 따로 답을 적어 둡니다.
짧게 답하면 **앞쪽은 조건부로 가능하고, 뒤쪽은 Azure가 주는 기능만으로는 성립하지 않습니다.**

**클라이언트 수정 없이** 부터 봅니다. AMR 데이터베이스를 `EnterpriseCluster` 정책으로 만들면
프록시가 클러스터를 감춰 주므로 기존 비클러스터 클라이언트가 그대로 붙습니다. 조건은 시점입니다 —
**생성할 때** 정해야 하고, 나중에 바꾸려면 DB를 지워야 합니다
([ACR과 AMR의 차이 2절](migration-guide/01-differences.md#2-왜-다른가--제품-계보와-클러스터-정책)).

그런데 정책으로 덮이지 않는 것이 셋 남습니다. **다중 DB는 정책과 무관하게 코드를 고쳐야 합니다** —
AMR에는 0번 하나뿐이라 프록시가 대신해 줄 것이 없습니다. 다중 키 명령은 허용 목록 6개 밖이면
`EnterpriseCluster`에서도 그대로 실패하고, 키스페이스 알림은 문서상 미지원인데 실측으로는 동작해서
판단이 한 겹 더 필요합니다. 이 셋을 코드에서 찾아내는 방법이
[클라이언트·SDK 확인사항](migration-guide/02-client-audit.md)입니다.

**다운타임 없이** 는 답이 다릅니다. 이름 때문에 기대하게 되는 Azure 마이그레이션 도구는
엔드포인트만 넘기고 **데이터를 옮기지 않습니다**
([이관 경로와 실측 3절](migration-guide/03-migration-paths.md#3-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다)).
그러면 복제를 걸어 타깃이 따라오게 하는 정석적인 방법이 남는데, `REPLICAOF`가
**소스와 타깃 양쪽에서** 막혀 있어 시작조차 못 합니다
([이관 경로와 실측 4.1절](migration-guide/03-migration-paths.md#41-가장-먼저-떠오르는-방법-그리고-왜-막히는가)).
물리적 복제를 전제로 한 무중단 계획은 이 지점에서 전부 지워집니다.

남는 것은 다운타임을 없애는 쪽이 아니라 **줄이는 쪽**입니다. 이 랩 규모(3.77GB / 215만 키)에서
복사 방식의 하한은 **약 111초**로 측정됐습니다
([이관 경로와 실측 2.3절](migration-guide/03-migration-paths.md#23-반복-복사로-유실이-얼마나-줄어드나)).
그보다 짧게 하려면 Azure 바깥으로 — 논리적 복제나 애플리케이션 계층으로 — 내려가야 합니다
...
세 이유가 공통으로 가리키는 것은 **통제권**입니다. 언제 넘길지, 무엇부터 넘길지, 넘기기 전에 확인할지를
직접 정할 수 있다는 것이 Option 1이 주는 것입니다. 그 통제권을 실제로 행사하는 방법이 아래 세 경로이고,
이 랩이 측정한 것은 그중 경로 B입니다.
...
경로 C가 이 표에서 가장 자주 오해받는 자리입니다. 이름과 달리 데이터는 그대로 두고 호스트 이름만 옮기므로,
이 경로를 고르더라도 **데이터 이관은 A나 B로 따로 해야 합니다.**
...
순위를 매기는 기준은 여러 가지일 수 있습니다 — 영향 범위, 발생 확률, 복구 비용, 눈에 띄는 정도.
여기서는 **되돌릴 수 있는가** 하나로 줄였습니다. 마이그레이션은 창이 정해져 있고
그 안에서는 복구 가능성이 나머지 기준을 전부 압도하기 때문입니다.
