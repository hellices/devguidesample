...
3. **용량** — ACR 크기를 AMR에 1:1로 매핑하면 모자랍니다 ([10절 상세](amr-migration-paths.md#10-용량-산정--두-번-속습니다))
...
| | 결론 | 상세 |
|---|---|---|
| 클러스터 | AMR은 **SKU 무관 항상 클러스터**. 안 쓰던 워크로드도 크로스 슬롯 제약을 받습니다 | [1.1절](amr-differences.md#11-기능-차이--엔진-샤딩-명령어-클라이언트) |
| 다중 키 명령 | `EnterpriseCluster`에서도 **6개만** 허용 (`DEL` `MSET` `MGET` `EXISTS` `UNLINK` `TOUCH`) | [2.5절](amr-differences.md#25-enterprisecluster도-크로스-슬롯-제약이-남습니다) |
| 데이터베이스 | **0번 하나뿐.** `SELECT`/`SWAPDB`와 연결 문자열의 DB 번호를 걷어내야 합니다 | [3.4절](amr-client-audit.md#34-tier-1--정책과-무관하게-반드시-고쳐야-하는-것) |
| 연결 | 포트 **10000**, 호스트명 `<name>.<region>.redis.azure.net` | [3.1절](amr-client-audit.md#31-연결-설정--무조건-바뀌는-것) |
| TLS | AMR은 **생성 시 TLS/비TLS 중 하나만** 선택. ACR처럼 혼용할 수 없습니다 | [1.1절](amr-differences.md#11-기능-차이--엔진-샤딩-명령어-클라이언트) |
| 네트워크 | **VNet 주입·IP 방화벽 규칙 미지원.** Private Link로 전환해야 합니다 | [1.1절](amr-differences.md#11-기능-차이--엔진-샤딩-명령어-클라이언트) |
| 복제 | `REPLICAOF`는 **소스·타깃 양쪽에서 차단**. 물리 복제 기반 전략은 불가 | [8.1절](amr-migration-paths.md#81-가장-먼저-떠오르는-방법-그리고-왜-막히는가) |
...
| 공식 가이드 | 이 묶음 |
|---|---|
| ① [Understand the differences](https://learn.microsoft.com/azure/redis/migrate/migrate-basic-standard-premium-understand) | [ACR과 AMR의 차이](amr-differences.md) · [클라이언트·SDK 확인사항](amr-client-audit.md) |
...
| ③ [Plan execution — self-service](https://learn.microsoft.com/azure/redis/migrate/migrate-basic-standard-premium-self-service) / [with tooling](https://learn.microsoft.com/azure/redis/migrate/migrate-basic-standard-premium-with-tooling) | 이 문서 [6절](#6-권장-절차) · [이관 경로와 실측](amr-migration-paths.md) |

### 상세 문서

| 문서 | 담긴 것 | 언제 보나 |
|---|---|---|
| [ACR과 AMR의 차이](amr-differences.md) | 기능·명령어·클라이언트 차이, `clusteringPolicy` 세 가지, **정책 × 클라이언트 실측 호환성 매트릭스**, 정책 변경 규칙 | 정책을 고를 때 |
| [클라이언트·SDK 확인사항](amr-client-audit.md) | TIER 1~4 체크리스트, SDK별 확인 포인트, 명령어 감사 스크립트, 정적 스캔의 한계 | 코드를 감사할 때 |
| [이관 경로와 실측](amr-migration-paths.md) | 경로 A/B/C, 실시간 전략 비교, 용량 산정, 테스트 환경과 재현, 측정하지 않은 것 | 실제로 옮길 때 |

### 목적별 읽기 경로

- **아직 고르는 중이다** → 이 문서 [0절](#0-요약) → [2절](#2-무엇을-고를-것인가--clusteringpolicy) → [ACR과 AMR의 차이](amr-differences.md)
- **코드를 감사해야 한다** → [클라이언트·SDK 확인사항](amr-client-audit.md) 전체
- **날짜를 잡고 실행한다** → 이 문서 [5절](#5-우선순위와-순서) → [6절](#6-권장-절차) → [이관 경로와 실측](amr-migration-paths.md)
- **왜 무중단이 안 되는지 설명해야 한다** → 이 문서 [3절](#3-클라이언트-수정-없이-다운타임-없이에-대한-답) → [8절 상세](amr-migration-paths.md#8-실시간-마이그레이션-전략--replicaof는-왜-안-되는가)
...
([2.6절](amr-differences.md#26-정책-변경은-nocluster에서-나오는-방향만-됩니다)). 무엇을 고를지는 취향이 아니라
...
| 조건 | 정책 | 클라이언트 작업 |
|---|---|---|
| 다중 키 명령을 거의 안 쓰고, 클러스터 클라이언트를 쓸 수 있다 | `OSSCluster` | 클러스터 전용 클라이언트로 교체 ([3.2절](amr-client-audit.md#32-sdk별-확인-포인트)) |
...
**첫 칸을 채우는 방법이 [3.3절](amr-client-audit.md#33-명령어-감사--자동-스캔)의 명령어 감사입니다.**
스크립트가 TIER 2(허용 목록 밖)와 TIER 3(허용 목록 6개)을 세어 주므로, 그 두 숫자가 위 표의 입력값이 됩니다.
정책별로 무엇이 통과하고 무엇이 막히는지는 [2.4절](amr-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)에서 실제로 돌려 봤습니다.
...
> (실측: [2.4절](amr-differences.md#24-실측-정책--클라이언트-조합별-명령-호환성)).
...
| 요구 | 답 | 근거 |
|---|---|---|
| 클라이언트 코드 수정 없이 | 대체로 가능하다. AMR 데이터베이스를 `EnterpriseCluster` 정책으로 **생성할 때** 정해야 한다 | [ACR과 AMR의 차이](amr-differences.md#2-acr과-amr은-무엇이-다른가) |
| 그래도 확인할 것 | 다중 DB는 정책과 무관하게 코드를 고쳐야 한다. 키스페이스 알림은 문서상 미지원이나 실측으로는 동작했다 — 의존 중이라면 따로 판단이 필요하다. 다중 키 명령은 허용 목록 6개 밖이면 확인이 필요하다 | [클라이언트·SDK 확인사항](amr-client-audit.md#3-클라이언트sdk-확인사항) |
| 다운타임 없이 | Azure 기능만으로는 불가능하다. 마이그레이션 도구는 데이터를 옮기지 않는다 | [7절](amr-migration-paths.md#7-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다) |
| 복제로 따라붙게 하면? | `REPLICAOF`는 소스·타깃 양쪽에서 차단된다. 물리적 복제 기반 전략은 쓸 수 없다 | [8.1절](amr-migration-paths.md#81-가장-먼저-떠오르는-방법-그리고-왜-막히는가) |
| 부득이한 다운타임 최소화 | 이 랩 규모(3.77GB / 215만 키)에서 실측 약 111초가 복사 방식의 하한. 더 줄이려면 논리적 복제나 애플리케이션 계층 전략 | [6.3절](amr-migration-paths.md#63-반복-복사로-유실이-얼마나-줄어드나) · [8절](amr-migration-paths.md#8-실시간-마이그레이션-전략--replicaof는-왜-안-되는가) |

한 가지만 덧붙이면, 복사가 도는 동안 소스에 들어온 쓰기는 그대로 두면 타깃에 반영되지 않습니다.
이 랩에서는 단일 패스 기준 48.47%가 그렇게 빠졌고, **키 개수 비교로는 드러나지 않았습니다**
([6.2절](amr-migration-paths.md#62-그런데-복사-중-들어온-쓰기의-4847가-사라집니다)).
...
| 경로 | 공식 문서 대응 | 한 줄 |
|---|---|---|
| [경로 A](amr-migration-paths.md#5-경로-a-rdb-export--import) RDB Export/Import | Option 1의 export/import | 원본이 **Premium일 때만** 가능 |
| [경로 B](amr-migration-paths.md#6-경로-b-scan--dumprestore-프로그래매틱-복사) SCAN + DUMP/RESTORE | Option 1의 데이터 이관 | 전 SKU 가능. **이 랩이 측정한 경로** |
| [경로 C](amr-migration-paths.md#7-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다) 마이그레이션 도구 | Option 2 (preview) | **데이터를 옮기지 않습니다.** 엔드포인트 전환만 |
| [8절](amr-migration-paths.md#8-실시간-마이그레이션-전략--replicaof는-왜-안-되는가) 실시간 복제 | (해당 없음) | 공식 경로가 아님. 막힌 이유를 정리한 절 |

**Option 2는 아래를 전부 만족할 때만** 검토하세요. 하나라도 걸리면 Option 1입니다.

- 프라이빗 엔드포인트·VNet 주입·지역 복제를 쓰지 않는다
- 전환 시점을 직접 고르지 않아도 된다
- 그 Redis에 붙은 **모든** 애플리케이션이 동시에 넘어가도 된다
- 데이터는 별도로 옮긴다는 것을 이해했다
...
관심사별 상세 판단 기준은 [이관 경로와 실측](amr-migration-paths.md)에 있습니다.
...
1. **애플리케이션 명령어를 감사합니다.** ([클라이언트·SDK 확인사항](amr-client-audit.md))
...
> 이중 쓰기 말고 다른 선택지도 있습니다 — [8절](amr-migration-paths.md#8-실시간-마이그레이션-전략--replicaof는-왜-안-되는가)에서
...
2. 이중 쓰기가 도는 상태에서 **과거 데이터를 복사**합니다. 이 시점부터의 신규 쓰기는 이미 양쪽에 들어가므로,
   [6.2절](amr-migration-paths.md#62-그런데-복사-중-들어온-쓰기의-4847가-사라집니다)의 유실 구간이 사라집니다.
3. 복사 후 검증합니다.
...
호스트 이름을 유지하고 싶고 [7절](amr-migration-paths.md#7-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다)의 제약을 모두 받아들일 수 있을 때만 고려하세요.
...
**이 문서가 측정하지 않은 것**과 재현 절차는 [이관 경로와 실측](amr-migration-paths.md#12-이-문서가-측정하지-않은-것)에 있습니다.
