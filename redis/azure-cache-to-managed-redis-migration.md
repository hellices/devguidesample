...
**기본 정책은 공식 문서의 Option 1(자체 마이그레이션)입니다.**
[Migration options](https://learn.microsoft.com/azure/redis/migrate/migrate-basic-standard-premium-options)가
대부분의 고객에게 Option 1을 권장하고, 이 절은 그 Option 1을 이 랩에서 실행 가능한 명령 단위로 편 것입니다
([4.6절](#46-우선순위--순서)의 순서를 따릅니다).

Option 2(마이그레이션 도구)는 **아래 조건을 전부 만족할 때만** 검토하세요. 하나라도 걸리면 Option 1입니다.

- 프라이빗 엔드포인트·VNet 주입·지역 복제를 쓰지 않는다
- 전환 시점을 직접 고르지 않아도 된다
- 그 Redis에 붙은 **모든** 애플리케이션이 동시에 넘어가도 된다
- 데이터는 별도로 옮긴다는 것을 이해했다 ([7절](#7-경로-c-azure-마이그레이션-도구는-데이터를-옮기지-않는다))
