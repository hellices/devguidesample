...
이 둘은 거의 항상 함께 요구받는다. 그래서 따로 답을 적어 둔다.
짧게 답하면 **앞쪽은 조건부로 가능하고, 뒤쪽은 Azure가 주는 기능만으로는 성립하지 않는다.**

**클라이언트 수정 없이** 부터 본다. AMR 데이터베이스를 `EnterpriseCluster` 정책으로 만들면
...
그런데 정책으로 덮이지 않는 것이 셋 남는다. **다중 DB는 정책과 무관하게 코드를 고쳐야 한다** —
AMR에는 0번 하나뿐이라 프록시가 대신해 줄 것이 없다. 다중 키 명령은 허용 목록 6개 밖이면
`EnterpriseCluster`에서도 그대로 실패하고, 키스페이스 알림은 문서상 미지원인데 실측으로는 동작해서
판단이 한 겹 더 필요하다. 이 셋을 코드에서 찾아내는 방법이
[클라이언트·SDK 확인사항](migration-guide/02-client-audit.md)이다.

**다운타임 없이** 는 답이 다르다. 이름 때문에 기대하게 되는 Azure 마이그레이션 도구는
엔드포인트만 넘기고 **데이터를 옮기지 않는다**
