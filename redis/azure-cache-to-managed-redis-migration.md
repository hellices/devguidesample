...
그래서 먼저 정할 것이 있다. 앞의 결정이 뒤를 제약하므로 순서도 이대로다.
...
숫자부터 보면 이 마이그레이션의 모양이 대강 잡힌다.
쓰기를 멈추면 2분 안에 무손실로 끝난다. 멈추지 않으면 절반가량을 잃는다.
그 사이의 중간 지대는 생각보다 좁다. 그것이 아래 표의 요지다.
...
유실은 이렇게 조용히 지나간다. 이 문서에서 하나만 가져간다면 이 대목이다.
측정 방법과 원본 데이터는 [이관 경로와 실측](migration-guide/03-migration-paths.md)에 있다.

### 반드시 알고 갈 차이

다른 소프트웨어라는 사실은 실무에서 이런 모습으로 나타난다. 아래에 한 화면으로 모았다.
첫 줄이 나머지를 거의 다 설명한다 — AMR은 SKU와 무관하게 항상 클러스터다.
그래서 클러스터를 켜 본 적 없는 워크로드도 크로스 슬롯 제약을 받는다.
...
Microsoft의 공식 방법론은 3단계다 — 차이를 이해하고 경로를 고른 뒤 실행 계획을 세운다.
이 묶음은 각 단계에 그대로 대응하면서 공식 문서가 숫자로 적지 않은 것을 채운다.
실제로 재 봤을 때의 다운타임과 유실률, 정책·클라이언트 조합별 실패 방식, 막다른 길로 확인된 접근을 넣었다.
...
조건별로 어떻게 갈리는지는 아래 표에 폈다. 자기 경우에 해당하는 한 줄만 보면 된다.
...
첫 칸은 [클라이언트·SDK 확인사항 3절](migration-guide/02-client-audit.md#3-명령어-감사--자동-스캔)의 명령어 감사로 채운다.
...
결론부터 적으면 앞쪽은 조건부로 가능하다. 뒤쪽은 Azure가 주는 기능만으로는 성립하지 않는다.

클라이언트 무수정부터 본다. AMR 데이터베이스를 `EnterpriseCluster` 정책으로 만들면
...
- **허용 목록 6개 밖의 다중 키 명령**: `EnterpriseCluster`에서도 그대로 실패한다.
  프록시가 감춰 주는 것은 엔드포인트다. 슬롯 경계는 그 뒤에 그대로 남아 있다.
- **키스페이스 알림**: 문서는 미지원이라고 하는데 이 랩에서는 동작했다.
  동작하는 것과 지원되는 것은 다르므로 판단이 한 겹 더 필요하다.

모두 정책 선택 이전의 문제다. 정책을 고르기 전에 코드에 이것이 있는지부터 확인해야 한다.
확인 목록은 [클라이언트·SDK 확인사항](migration-guide/02-client-audit.md)에 있다.

다운타임 0은 사정이 다르다. 무중단을 만들어 줄 수 있었을 방법이 둘 있다. 둘 다 막혀 있다.
...
물리적 복제에 기대는 무중단 계획은 이 지점에서 전부 지워진다.

남는 것은 다운타임을 없애는 쪽이 아니라 줄이는 쪽이다. 이 랩 규모(3.77GB / 215만 키)에서
...
한 가지만 덧붙인다. 복사가 도는 동안 소스에 들어온 쓰기는 그대로 두면 타깃에 반영되지 않는다.
이 랩에서는 단일 패스 기준 48.47%가 그렇게 빠졌다. 그런데 **키 개수 비교로는 드러나지 않았다**
([이관 경로와 실측 2.2절](migration-guide/03-migration-paths.md#22-복사-중-들어온-쓰기의-4847-유실)).

여기까지가 제약이다. 그 제약 안에서 어느 경로로 옮길지는 다음 절에서 다룬다.
...
여기서 권장하는 쪽은 **Option 1(자체 마이그레이션)**이다. 문서는 세 가지 이유를 든다.
...
세 이유는 모두 **통제권**을 가리킨다. Option 1에서는 언제 넘길지, 무엇부터 넘길지, 넘기기 전에 확인할지를
직접 정할 수 있다. 그 통제권을 실제로 행사하는 방법이 아래 세 경로이고
이 랩은 그중 경로 B를 측정했다.
...
경로 C는 이 표에서 가장 자주 오해받는다. 이 경로를 고르더라도 **데이터 이관은 A나 B로 따로** 해야 한다.
이름과 달리 데이터는 그대로 두고 호스트 이름만 옮기기 때문이다.
...
1. 유일하게 되돌릴 수 없다. 나머지 셋은 시간과 비용으로 복구된다. 정책을 잘못 골랐으면 DB를 다시 만들고
   용량이 모자라면 SKU를 올린다. 다운타임이 길면 다음 창을 잡으면 된다.
...
우선순위는 "충돌하면 무엇을 지키나"에 답한다. 순서는 "무엇을 먼저 하나"다. 두 축이 다르다.
...
[5.2절](#52-우선순위--순서)의 순서를 명령 단위로 폈다. 공식 문서의 Option 1을 따른다.
...
- `INCR`, `LPUSH`, `SETNX` 같은 **읽기-수정-쓰기 성격의 명령**은 이중 쓰기로 정합성이 깨질 수 있다.
  카운터나 큐로 Redis를 쓴다면 해당 키만 따로 처리해야 한다. 순수 캐시 용도라면 문제되지 않는다.
...
이 경우에도 **데이터는 6.1 또는 6.2로 별도 이관**해야 한다.
