...
| Bedrock | `${apiPathPrefix}/bedrock` | `POST /model/{modelId}/converse` (SigV4 서명은 APIM이 대행) |
...
- 정책은 호출자(caller)의 Entra bearer token을 백엔드로 전달하기 전에
  삭제(isolate)하고, Bedrock Runtime 요청 경로/쿼리/헤더/본문 해시를 이용해
  자체적으로 SigV4 서명을 생성한다.
