...
![작업 영역 구성 화면의 네트워킹 탭을 보여주는 미리 보기 화면입니다. 송신 모드로 Unrestricted, Limited, Azure VNet 세 가지 카드가 나란히 있고 그중 Azure VNet이 선택되어 연결됨 상태로 표시됩니다. 아래에는 연결한 위임 서브넷 경로와 함께 서브넷이 /28 이상이어야 하고 Microsoft.App/environments에 위임되어야 한다는 안내, 자동으로 켜진 VNet 프라이빗 DNS 사용 옵션, 연결 테스트 단추와 VNet 연결 해제 단추가 있습니다. 그 아래 인프라 네트워크 영역에는 이 경로가 VNet을 우회하므로 NSG 규칙과 프라이빗 DNS가 적용되지 않는다는 경고와 함께 MCP 서버 접근 토글, 패키지 관리자 접근 토글, GitHub와 GitHub Enterprise, Azure DevOps 코드 저장소 선택란, 그리고 와일드카드까지 입력할 수 있는 추가 호스트 입력란이 있습니다.](sre-agent-event-lab/assets/official/azure-sre-agent-networking-vnet.png)
...
| 추가 호스트 목록에 등록한 호스트 | 인프라 네트워크 | 호스트 이름이나 `*.example.com` 같은 와일드카드 패턴을 직접 입력해 관리합니다. 미리 설치한 패키지가 사용하는 호스트는 자동으로 허용되므로 그 밖의 대상만 등록하면 됩니다. |
...
![Operations Hub 개요 탭 화면입니다. 왼쪽 사이드바에는 조사 대화, 인시던트, 예약 작업, Builder, 모니터, 기능, 설정 메뉴가 있고, 오른쪽 본문에는 개요와 분석, 자동화 탭, 최근 7일 기간 선택, 원본별 일일 처리량 차트, 대기 중인 작업 41건 카드, 시스템 상태 카드가 배치되어 주요 지표를 한 화면에서 보여 줍니다.](sre-agent-event-lab/assets/official/operations-hub-overview-tab.png)
