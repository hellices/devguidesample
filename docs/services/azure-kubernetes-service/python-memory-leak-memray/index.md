...
applies_to:
- 공식 원문 재검토 필요
title: AKS Python Pod 메모리 누수 분석
description: kubectl debug와 Memray attach를 이용해 Python Pod 메모리 누수를 분석합니다.
technologies:
- kubernetes
- python
tags:
...
gdb + procps + memray를 담은 이미지를 레지스트리에 상비 ([debug/Dockerfile](https://github.com/hellices/devguidesample/blob/main/docs/services/azure-kubernetes-service/python-memory-leak-memray/samples/memray-leak-profiling/debug/Dockerfile)):
...
| 상시 프로파일링 | memray = 스팟 조사용. 상시는 [Pyroscope](../pyroscope-anf-s3/index.md) 등 continuous profiler 병행 |
...
재현 환경 전체 코드: [`memray-leak-profiling/`](https://github.com/hellices/devguidesample/tree/main/docs/services/azure-kubernetes-service/python-memory-leak-memray/samples/memray-leak-profiling)
...
앱 이미지는 clean 유지 ([Dockerfile](https://github.com/hellices/devguidesample/blob/main/docs/services/azure-kubernetes-service/python-memory-leak-memray/samples/memray-leak-profiling/Dockerfile)):
...
```bash
cd docs/services/azure-kubernetes-service/python-memory-leak-memray/samples/memray-leak-profiling
