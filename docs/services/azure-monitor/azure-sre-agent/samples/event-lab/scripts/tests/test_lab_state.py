...
def test_every_scenario_has_a_guide_to_send_an_operator_to():
    ...
    expected = {
        "s1": "docs/services/azure-monitor/azure-sre-agent/scenario-http-500/index.md",
        "s2": "docs/services/azure-monitor/azure-sre-agent/scenario-latency/index.md",
        "s3": "docs/services/azure-monitor/azure-sre-agent/scenario-blob-permission/index.md",
    }
...
def test_the_refusal_lists_every_blocker_earliest_first(tmp_path):
    ...
    assert "docs/services/azure-monitor/azure-sre-agent/scenario-blob-permission/index.md" in message
...
def test_the_refusal_names_the_blocking_scenario_its_status_and_a_remedy(tmp_path):
    ...
    assert "docs/services/azure-monitor/azure-sre-agent/scenario-latency/index.md" in message, message
...
def test_the_ordered_remedy_never_tells_an_operator_to_restart_a_running_run(tmp_path):
    ...
    assert "docs/services/azure-monitor/azure-sre-agent/scenario-http-500/index.md" not in message, message
    assert "mark-failed s1" in message, message
...
@pytest.mark.parametrize(
    ...
    (
        (
            "running",
            "mark-failed s1",
            "docs/services/azure-monitor/azure-sre-agent/scenario-http-500/index.md",
        ),
        (
            "failed",
            "docs/services/azure-monitor/azure-sre-agent/scenario-http-500/index.md",
...
def test_cli_evidence_dir_without_a_run_names_the_command_to_run(tmp_path):
    ...
    assert result.returncode == 1
    assert "docs/services/azure-monitor/azure-sre-agent/scenario-http-500/index.md" in result.stderr
    assert "Traceback" not in result.stderr
...
def test_cli_require_run_refuses_every_scenario_while_one_run_is_unfinished(tmp_path):
    ...
    for scenario in ("s1", "s2"):
        ...
        assert "docs/services/azure-monitor/azure-sre-agent/scenario-blob-permission/index.md" in refused.stderr
