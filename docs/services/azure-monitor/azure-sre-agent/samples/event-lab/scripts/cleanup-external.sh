...
' "${CLEANUP_SETUP_FILE}" 2>/dev/null)"; then
  ...
  echo "Rewrite evidence/agent-setup.json as shown in docs/services/azure-monitor/azure-sre-agent/setup/index.md, then run: python3 scripts/lab_state.py acknowledge-agent" >&2
  exit 1
fi
...
while IFS='|' read -r assignment_key assignment_id principal_key expected_principal_id; do
  ...
  if [[ -z "${assignment_id}" ]]; then
    ...
    echo "Update evidence/agent-setup.json as shown in docs/services/azure-monitor/azure-sre-agent/setup/index.md, then run: python3 scripts/lab_state.py acknowledge-agent" >&2
    exit 1
  fi
