...
title: SRE Agent Blob 권한 장애 시나리오
description: Blob 권한을 제거하고 SRE Agent의 권한 문제 진단을 확인합니다.
technologies:
- python
tags:
- ai-agents
...
- [S2 — 응답 지연](../scenario-latency/index.md)의 S2가 복구되고 캡처가 `conclusion`으로 끝났습니다.
...
역할 전파에는 몇 분이 걸릴 수 있습니다. `/api/documents`가 200을 돌려주는지 직접 호출해 확인하고, 실패로 기록되었다면 이 절을 `begin-run`부터 다시 실행해 새 시도를 시작합니다. 다시 실행하면 이전 시도의 `s3_recovered`와 `s3_captured` 기록이 주입 전에 지워지므로, 새 시도가 복구되고 `record-capture`될 때까지 채점은 막힙니다.
...
수집한 근거를 채점합니다: [결과 채점](../results/index.md)
