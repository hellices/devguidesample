...
- azure-hdinsight
official_sources:
- title: Azure HDInsight documentation
  url: https://learn.microsoft.com/azure/hdinsight/
document_type: research
status: current
verification_status: needs-review
sources_checked_at: 2026-09-12
title: HDInsight Kafka Catch-up 성능 벤치마크
description: HDInsight Kafka broker SKU와 consumer fetch size에 따른 catch-up 성능을 비교합니다.
technologies:
- kafka
tags:
- benchmarking
...
- Date: 2026-07-16 | Region: japaneast | Cluster: `hdi-example-japaneast-01` (HDInsight Kafka, broker 3)
...
- Cluster `hdi-example-japaneast-01`: deleted (D4·D8·D16 각 테스트 후 재생성/삭제).
- VM `vm-kafka-producer`, `vm-kafka-loadgen`, `vm-jumpbox`, `vm-monitoring`: deallocated/삭제.

```bash
az hdinsight delete -g rg-example-japaneast-01 -n hdi-example-japaneast-01 --yes
# 전체 정리: az group delete -n rg-example-japaneast-01 --yes
```
...
- [HDInsight Kafka 모니터링 방식 비교](../index.md) — Azure Monitor·Log Analytics·Ambari·진단 설정 개요.
- [Prometheus + Grafana 대시보드 구성](../prometheus-grafana/index.md) — broker JMX·kafka-exporter(partition별 lag) 대시보드 구성.
