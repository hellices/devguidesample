...
title: Agent Memory 프로덕션 운영과 평가
description: Agent memory를 프로덕션에서 운영하고 품질을 평가하는 기준을 정리합니다.
technologies:
- rag
tags:
- ai-agents
...
4. 기본 관측성을 붙인다                 ← 검색 지연·추출 실패 로깅부터. 대시보드·비용 추적은 6번과 함께
...
이 순서는 [06. 커머스 적용 설계 §10 로드맵](../agent-memory-commerce/index.md)의 Phase 0(1~4) → Phase 2(5) → Phase 3(6) → Phase 4(7)에 그대로 대응한다.
...
- [06. 커머스 적용 설계](../agent-memory-commerce/index.md)
