...
title: 'A10 vs T4: 처리량과 비용으로 고르는 임베딩 GPU'
description: BGE-m3-ko와 TEI를 사용한 실측입니다. 단건 지연, 배치 처리량, 청크 길이를 나눠 보고 GPU 선택과 증설의 기준을 비교합니다.
featured: true
technologies:
- python
tags:
...
> 관련 문서: [GPU vLLM RAG 가이드](../../../guides/azure-ai-search/gpu-vllm-rag/index.md) | [Custom 임베딩 적재 가이드](../../../guides/azure-ai-search/custom-embedding-ingestion/index.md) | [BGE-M3 vs Qwen3 품질 비교](../bge-m3-vs-qwen3-embedding/index.md)
>
> 작성일: 2026-07-22
...
- **쿼리 경로(단건 추론)**: 양쪽 모두 single-digit ms — latency-bound 구간으로, 전체 검색 응답 시간에서 차지하는 비중이 상대적으로 작음. QPS 스윕 실측 기준 T4 1대는 **~200 QPS(40자 쿼리)까지 p50 ~6ms 유지, 초과 시 큐잉 지연 — 안전 운용 상한 ~150 QPS/대** → [증설 기준](#qps)
...
- 공개 스펙 교차검증 — FP16 Tensor TFLOPS비 1.9×, Flash Attention 지원 차이, 메모리 대역폭 2×가 **주요 원인으로 추정**되며, 공개 스펙 기반 예측 범위(4.3~5.8×)와 실측 결과의 방향성이 일치 → [교차검증 섹션](#_4)
- 대체 전략: A10 수급 불가 시, 본 측정 조건 기준 **A10 × N ≈ T4 × 5N**(500자) / **× 6N**(1,000자) 수준으로 대체 산정 — 최소 SKU(NC4as) × N 구성 시 VM 비용 기준 등가 처리량 확보 가능 → [대체 전략](#a10-t4)
...
- 실행 스크립트: [bench/](https://github.com/hellices/devguidesample/tree/main/samples/azure-ai-search/source-material/custom_vectorization/bench)
...
## 대체 전략: A10 수급 불가 시 T4 스케일아웃

> A10 물량 확보가 불가한 경우, 동등 처리량을 확보하기 위한 T4 대체 구성 산정 기준. vCPU 교차검증(GPU compute-bound 확인)이 근거 — T4 계열 최소 SKU(NC4as_T4_v3, 4 vCPU)로도 GPU 처리량은 동일하므로 **처리량/달러 최적점 = 최소 SKU × N**.
...
대체 구성 시 비용이 오히려 늘지 않는지 확인 목적. Azure Retail Prices API 조회 (2026-07, southcentralus 기준 — 리전별 단가 상이): NC4as_T4_v3 $0.631/hr, NV36ads_A10_v5 $3.84/hr

| 구성 | 시간당 (southcentralus) | A10 등가 대비 |
|------|------------------------|---------------|
| A10 × 2 (기본 구성) | $7.68/hr | 기준 |
| T4 × 10 (500자 등가 대체) | $6.31/hr | 약 -18% |
| T4 × 12 (1,000자 등가 대체) | $7.57/hr | -1% (사실상 동률) |

> 위 절감률은 **VM 비용만 기준으로 관찰된 수치**다. 노드 수 증가에 따른 운영비(모니터링, 패치, 장애 대응 인력)를 포함한 TCO는 환경별로 재산정이 필요하다. **결론: 대체 구성이라도 VM 비용은 등가 이하 — 비용이 대체를 막는 요인은 아님.**

- LB는 A10 × 2 기본 구성에도 이미 존재하는 공통 요소 — 비교 시 비용 차이 없음
- 노드 수 증분 부대비용: OS 디스크(P10급) ~$0.02/hr × 추가 노드 수 → T4 10대 기준 +$0.2/hr 수준 (무시 가능)
- 상시 운영이면 1-year Savings Plan/RI 적용 시 30~60% 추가 절감 여지 (양쪽 공통)

### 엔드포인트 아키텍처

기존 A10 × 2 구성의 LB backend pool을 T4 노드로 교체/확장하는 형태 — TEI는 stateless HTTP(`POST /embed`)라 L4 분배로 충분:
...
### 쿼리 트래픽 증가 시 증설 기준 — QPS 스윕 실측

T4 1대(NC4as_T4_v3, TEI turing-1.8, BGE-m3-ko)에 오픈루프 고정 도착률로 단건 쿼리(~40자)를 주입하며 포화 지점을 실측 (30s/단계, 3라운드 스윕 — [bench_qps_sweep.py](https://github.com/hellices/devguidesample/blob/main/samples/azure-ai-search/source-material/custom_vectorization/bench/bench_qps_sweep.py)):

| 도착률 (QPS) | p50 | p95 | p99 | GPU util | 판정 |
|-------------|-----|-----|-----|----------|------|
| 50 | 9.2 ms | 9.5 ms | 10.0 ms | 39% | 여유 |
| 100 | 5.6 ms | 6.0 ms | 8.7 ms | 41% | 여유 |
| 150 | 5.6 ms | 5.8 ms | 9.4 ms | 64% | **안전 운용 상한 권장** |
| 180 | 6.0 ms | 6.5 ms | 12.5 ms | 76% | 여유 소진 접근 |
| **200** | **6.0 ms** | 6.9 ms | 19.2 ms | **90%** | **무릎(knee) — 큐잉 직전** |
| 210 | 15.1 ms | 19.2 ms | 27.5 ms | 95% | 큐잉 시작 (p50 2.5×) |
| 250 | 24.0 ms | 28.1 ms | 29.7 ms | 97% | 큐잉 지연 지속 |
| 400 | 74.0 ms | 93.9 ms | 128 ms | 99% | 과부하 |
| 450+ | 557~896 ms | 900~965 ms | 937~983 ms | 100% | 폭주 (수용 불가) |

**핵심 관찰:**
- **무릎 지점: ~200 QPS (GPU util ~90%)** — 이 지점까지 p50은 베이스라인(~6ms) 유지, 초과 즉시 p50이 2.5× 점프(6→15ms)하며 큐잉 지연 시작. 도착률에 비례해 지연 누적, 450 QPS부터 지수적 폭주
- **안전 운용 상한: ~150 QPS/대 (GPU ~64%, p99 <10ms)** — 무릎 대비 25% 마진
- 단문 쿼리(40자)의 상한(200 QPS)은 500자 청크 처리량(60 texts/s)의 **3.3배** — attention O(N²)·토큰 수 비례로 per-query 연산량이 낮기 때문. 즉 **쿼리 경로와 적재 경로의 노드당 수용량은 별도 산정 필요**
- 저부하(5~50 QPS) 구간의 mean 9~10ms > 100 QPS의 5.6ms — GPU idle clock(power 30W)에서 dynamic batching 미형성 시 오히려 느림. 부하 증가 시 clock boost + 배치 효율로 개선 (레이턴시-부하 관계가 단조 증가가 아님)

**증설 트리거 (실측 기반):**

| 지표 | 임계값 | 근거 |
|------|--------|------|
| 노드당 쿼리 QPS | > 150 (40자 기준) | 무릎(200 QPS) 대비 25% 마진 |
| GPU utilization (지속) | > 70~80% | 90%가 무릎 — 선제 증설 구간 |
| 임베딩 p50 레이턴시 | 베이스라인의 2× (≈12ms+) | 무릎 초과 시 p50 2.5× 점프 실측 |
| 임베딩 p99 레이턴시 | > 20ms | 200 QPS에서 p99 19ms — 큐잉 전조 |

- p50 기반이 가장 민감 — 무릎 초과가 p50에 즉시 반영됨 (p99는 무릎 전부터 완만히 상승해 노이즈 있음)
- VMSS 구성 시 GPU util 70% 기준 autoscale 룰로 자동화 가능
- **쿼리·적재 혼용 주의**: 동일 풀에서 Push 적재(대형 배치)와 쿼리를 겸용하면 적재 배치의 GPU 점유로 쿼리 레이턴시가 상승. 이 경우 증설보다 ① 적재 시간대 분리 또는 ② backend pool 분리(쿼리 전용 + 적재 전용)가 우선 — 쿼리 단독으로 150 QPS/대를 초과하는 경우보다 적재 간섭이 원인인 경우가 흔함

> 측정 조건: 단일 노드 localhost 호출(네트워크 홉 제외), ~40자 한국어 쿼리 고정. 쿼리 길이가 길어지면 상한이 낮아지며(500자 기준 ~60 QPS), LB 경유·실트래픽 분포에서는 무릎이 다소 앞당겨질 수 있음.

### 대체 구성 판단

| 관점 | T4 × 5N (대체 구성) | A10 × N (기본 구성) |
|------|--------------------|---------------------|
| 시간당 VM 비용 (500자) | 약 -15~18% (등가 이하) | 기준 |
| 시간당 VM 비용 (1,000자) | 동률 (6N대 필요) | 기준 |
| 수급 | ✅ T4는 구세대로 상대적 수급 안정 | Spot/리전에 따라 물량 변동 |
| 장애 내성 | 노드당 1/5N 손실 | 노드당 1/N 손실 (N≥2면 SPOF 없음) |
| 운영 복잡도 | 관리 포인트 5N (probe·드라이버 패치) | ✅ 관리 포인트 N |
| 스케일 증분 | ✅ 60 texts/s 단위 미세 조정 | 301 texts/s 단위 |

**정리:** A10 물량 확보가 불가한 시점에는 **T4 × 5N(500자 기준, 1,000자 위주면 6N) 구성으로 처리량·비용 모두 등가 수준 확보 가능**. LB 등 엔드포인트 구조는 기존과 동일하므로 backend pool 교체만으로 전환 가능. 노드 수 증가에 따른 관리 포인트 확대는 VMSS 자동화로 완화. 실환경 편차(LB 분배·warmup·eviction)를 감안해 여유분 +1대 이상 산정을 권장.
...
- 1M+ 청크 적재: A10 1대 ≈ T4 5대 — A10 수급 불가 시 T4 × 5N으로 대체 산정 → [대체 전략](#a10-t4) 참고
