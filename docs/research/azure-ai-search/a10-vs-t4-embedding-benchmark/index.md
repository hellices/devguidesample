...
title: A10과 T4 임베딩 추론 벤치마크
description: Azure Spot VM에서 A10과 T4 GPU의 BGE 임베딩 처리 성능을 비교합니다.
technologies:
- python
tags:
- benchmarking
...
- **쿼리 경로(단건 추론)**: 양쪽 모두 single-digit ms — latency-bound 구간으로, 전체 검색 응답 시간에서 차지하는 비중이 상대적으로 작음. QPS 스윕 실측 기준 T4 1대는 **~200 QPS(40자 쿼리)까지 p50 ~6ms 유지, 초과 시 큐잉 지연 — 안전 운용 상한 ~150 QPS/대** → [증설 기준](#쿼리-트래픽-증가-시-증설-기준--qps-스윕-실측)
...
- 대체 전략: A10 수급 불가 시, 본 측정 조건 기준 **A10 × N ≈ T4 × 5N**(500자) / **× 6N**(1,000자) 수준으로 대체 산정 — 최소 SKU(NC4as) × N 구성 시 VM 비용 기준 등가 처리량 확보 가능 → [대체 전략](#대체-전략-a10-수급-불가-시-t4-스케일아웃)
...
- 1M+ 청크 적재: A10 1대 ≈ T4 5대 — A10 수급 불가 시 T4 × 5N으로 대체 산정 → [대체 전략](#대체-전략-a10-수급-불가-시-t4-스케일아웃) 참고
