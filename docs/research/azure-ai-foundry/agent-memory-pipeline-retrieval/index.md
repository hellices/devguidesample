...
- λ가 클수록 빨리 잊는다. **half-life는 도메인과 기억 유형에 따라 수십 분에서 수백 일까지 벌어진다** — 단일 값을 쓰면 안 된다. 유형별 권장치는 [01. 메모리 분류 체계의 Retention 정책 표](../agent-memory-taxonomy/index.md)와 [06. 커머스 적용 설계 §6](../agent-memory-commerce/index.md) 참조
...
| 실패 | 설명 | 대응 |
|------|------|------|
| 중요 정보 소실 | 계약 조건·법적 요구·안전 규칙이 decay로 프루닝됨 | **pin / decay 면제 목록**을 반드시 설계 |
...
> **핵심 조합 (기법 10 + 18)**: 사실에 `stable` / `volatile` 라벨을 붙여, stable(유저 이름·기기 호환 조건)은 decay를 우회하고 volatile(진행 중인 관심사·마감)만 정상 decay시킨다. 이 **selective decay**가 영구 정보가 파뭒히는 것을 막는다.
...
| 시간 가중 | **포함, 단 selective decay** | 약정·호환 조건은 pinned, 세션 의도는 짧은 half-life |
...
- [04. 프레임워크 비교](../agent-memory-frameworks/index.md) — 직접 만들 것인가 가져다 쓸 것인가
- [05. 프로덕션과 평가](../agent-memory-production-evaluation/index.md) — 이 파이프라인을 규모에서 운영하기
