...
applies_to:
- 공식 원문 재검토 필요
title: Codex와 Microsoft Foundry 폐쇄망 연동
description: 폐쇄망에서 Codex와 Microsoft Foundry를 연동할 때 필요한 네트워크 구성을 설명합니다.
technologies:
- rest-api
tags:
- networking
...
> 예: Private Endpoint 도메인이 `example-openai.openai.azure.com`인 경우

**Windows hosts 파일 경로**: `C:\Windows\System32\drivers\etc\hosts`

```
127.0.0.1  example-openai.openai.azure.com
```
...
name = "azure-openai"
base_url = "https://example-openai.openai.azure.com/openai/v1/"
...
http {
    ...
    server {
        listen       443;
        server_name  example-openai.openai.azure.com;

        location / {
            proxy_pass http://127.0.0.1:50056;

            proxy_set_header Host example-openai.openai.azure.com;
