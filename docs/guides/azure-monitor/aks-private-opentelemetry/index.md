...
applies_to:
- 공식 원문 재검토 필요
title: AKS 폐쇄망 OpenTelemetry와 Azure Monitor 구성
description: 폐쇄망 AKS에서 OpenTelemetry와 Azure Monitor 메트릭 파이프라인을 구축합니다.
technologies:
- kubernetes
- open-telemetry
tags:
...
| 항목 | 값 |
|------|-----|
| AKS 클러스터 | `aks-example-koreacentral-01` (Private Cluster) |
| 리소스 그룹 | `rg-example-koreacentral-01` |
| PRD 리소스 그룹 | `rg-example-koreacentral-prd-01` |
| 구독 | `<subscription-id>` |
| 리전 | Korea Central |
| VNet (기존) | `vnet-example-koreacentral-01` (`10.0.0.0/10`) |
| VNet (PRD) | `vnet-example-koreacentral-prd-01` (`10.100.0.0/16`) |
| AKS 서브넷 | `aks-subnet` (`10.1.0.0/24`) |
| PE 서브넷 | `pe-subnet` (`10.2.0.0/22`) |
| Azure Firewall | `fw-example-koreacentral-prd-01` (Private IP: `10.100.0.4`) |
| Azure Monitor Workspace | `azuremonitor-workspace-example-krc-01` |
| Managed Grafana | `grafanaworkspaceexample` |
...
flowchart TB
    ...
    subgraph AZURE["Azure (Private Link)"]
        subgraph AMPLS_SCOPE["AMPLS: ampls-example-krc-01"]
            DCE["DCE<br/>MSProm-koreacentral-<br/>aks-example-koreacentral"]
            DCE2["DCE<br/>azuremonitor-workspace-<br/>example-krc-01"]
        end
        PE["Private Endpoint<br/>pe-ampls-example-krc-01<br/>(pe-subnet)"]
    ...
    subgraph NET["네트워크 / DNS"]
        EXISTING_VNET["vnet-example-koreacentral-01<br/>10.0.0.0/10"]
        PRD_VNET["vnet-example-koreacentral-prd-01<br/>10.100.0.0/16"]
        FIREWALL["Azure Firewall<br/>fw-example-koreacentral-prd-01<br/>DNS Proxy: 10.100.0.4"]
...
2. 이름: `ampls-example-krc-01`
3. 연결된 리소스 (DCE 2개 추가):
   - `MSProm-koreacentral-aks-example-koreacentral` (RG: `rg-example-koreacentral-01`)
   - `azuremonitor-workspace-example-krc-01` (RG: `MA_azuremonitor-workspace-example-krc-01_koreacentral_managed`)
4. Private Endpoint 생성:
   - 이름: `pe-ampls-example-krc-01`
   - 서브넷: `pe-subnet`
...
|---|---|---|
| `privatelink.monitor.azure.com` | 메트릭 수집 엔드포인트 | `vnet-example-koreacentral-prd-01` |
| `privatelink.ods.opinsights.azure.com` | ODS 데이터 채널 | `vnet-example-koreacentral-prd-01` |
| `privatelink.oms.opinsights.azure.com` | OMS 포털 | `vnet-example-koreacentral-prd-01` |
| `privatelink.agentsvc.azure-automation.net` | 에이전트 서비스 | `vnet-example-koreacentral-prd-01` |
| `privatelink.blob.core.windows.net` | Blob 스토리지 | `vnet-example-koreacentral-prd-01` |
| `*.privatelink.koreacentral.azmk8s.io` | AKS API Server | `vnet-example-koreacentral-prd-01` |
| `privatelink.redis.azure.net` | Redis Cache | `vnet-example-koreacentral-prd-01` |
...
```bash
# AKS 파드 내부에서
nslookup msprom-koreacentral-aks-example-koreacentral-ruja.koreacentral-1.metrics.ingest.monitor.azure.com
# → 10.2.0.6 (Private IP ✅)
...
|---|---|---|
| `MSProm-koreacentral-aks-example-koreacentral-01` | `rg-example-koreacentral-01` | `MSProm-koreacentral-aks-example-koreacentral` |
| `azuremonitor-workspace-example-krc-01` | `MA_..._managed` | `azuremonitor-workspace-example-krc-01` |
...
1. Azure Portal → **Monitor** → **데이터 수집 규칙 (Data Collection Rules)**
2. Prometheus DCR 선택 (예: `MSProm-koreacentral-aks-example-koreacentral-01`)
...
```bash
az rest --method put \
  --url "/subscriptions/<subscription-id>/resourceGroups/rg-example-koreacentral-01/providers/Microsoft.ContainerService/managedClusters/aks-example-koreacentral-01/providers/Microsoft.Insights/dataCollectionRuleAssociations/configurationAccessEndpoint?api-version=2023-03-11" \
  --body '{"properties":{"dataCollectionEndpointId":"/subscriptions/<subscription-id>/resourceGroups/rg-example-koreacentral-01/providers/Microsoft.Insights/dataCollectionEndpoints/MSProm-koreacentral-aks-example-koreacentral"}}'
```

**확인:**
```bash
az rest --method get \
  --url "/subscriptions/<subscription-id>/resourceGroups/rg-example-koreacentral-01/providers/Microsoft.ContainerService/managedClusters/aks-example-koreacentral-01/providers/Microsoft.Insights/dataCollectionRuleAssociations?api-version=2023-03-11" \
  | python3 -c "
import sys,json
...
| **`configurationAccessEndpoint`** | N/A | **`MSProm-koreacentral-aks-example-koreacentral`** ✅ |
...
| 리소스 | 이름 | 설정 |
|---|---|---|
| Resource Group | `rg-example-koreacentral-prd-01` | Korea Central |
| VNet | `vnet-example-koreacentral-prd-01` | `10.100.0.0/16` |
| Subnet (Firewall) | `AzureFirewallSubnet` | `10.100.0.0/26` |
| Subnet (Default) | `default` | `10.100.1.0/24` |
| Azure Firewall | `fw-example-koreacentral-prd-01` | Standard SKU, DNS Proxy 활성화 |
| Firewall Private IP | `10.100.0.4` | DNS Proxy 엔드포인트 |
| Public IP | `pip-fw-example-koreacentral-prd-01` | Standard SKU |
...
```mermaid
flowchart TB
    VM["VM / AKS Pod<br/>(vnet-example-koreacentral-01)"] -->|"DNS Query"| RESOLVER["systemd-resolved<br/>127.0.0.53"]
    RESOLVER -->|"Forward to<br/>Custom DNS: 10.100.0.4"| FW["Azure Firewall<br/>DNS Proxy<br/>(vnet-example-koreacentral-prd-01)"]
...
기존 VNet(`vnet-example-koreacentral-01`)에 연결되어 있던 모든 Private DNS Zone 링크를 PRD VNet으로 이전.
...
az network private-dns link vnet create \
  ...
  --virtual-network "/subscriptions/.../vnet-example-koreacentral-prd-01" \
...
- Private Endpoint(`pe-ampls-example-krc-01`)는 기존 VNet의 `pe-subnet`에 그대로 위치
