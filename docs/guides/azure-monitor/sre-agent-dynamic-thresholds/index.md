...
applies_to:
- 공식 원문 재검토 필요
title: Dynamic Thresholds와 SRE Agent 연계
description: Azure Monitor Dynamic Thresholds 경고를 SRE Agent 조사 흐름과 연계합니다.
technologies:
- rest-api
tags:
- monitoring
...
- Event path(레거시 bridge): Dynamic alert → Action Group → Logic App managed identity → SRE HTTP Trigger → Review-mode investigation. 2026-08-12 실측에 쓰인 legacy 구성이고 기본 실습에는 배포하지 않는다([validation-results.md](../../../research/azure-monitor/sre-agent-validation-results/index.md)).
