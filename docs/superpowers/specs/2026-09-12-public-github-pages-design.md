...
`docs/superpowers/specs/`는 설계 기록용이며 공개 내비게이션과 검색 인덱스에서 제외한다. `docs-taxonomy.yml`은 서비스, 기술, 태그, 문서 유형과 상태의 허용 식별자를 관리하는 단일 기준 파일이다. 생성기와 검증기는 이 파일을 읽으며 같은 목록을 코드에 다시 하드코딩하지 않는다.
